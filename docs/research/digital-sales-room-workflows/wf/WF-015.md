...
- [ ] Technical design doc written and Jev-validated — written at
      `docs/design/WF-015-identity-and-domain-access.md`. Jev selects `ready`
      (p=0.78) but reports confidence 0.67 against the 0.75 bar, so the verdict is
      `uncertain` and was **not** overridden. See the escalation note in that doc
      and the recorded decisions in `orchestration/decisions/jev-audit.jsonl`.
- [x] Implemented
- [x] Tests written and passing — `backend/tests/test_access.py` (pure rules) and
      `backend/tests/test_access_api.py` (behaviour through HTTP)
- [ ] Reviewer bot passed
- [ ] Jev merge gate passed
- [ ] Verified in localhost browser — the headless equivalent
      (`tools/verify_localhost.py`, including a full gate round trip) passes, but
      no desktop browser was attached to this session, so the visual check is
      still outstanding.
